              ...
              {showQuickAdd && item.variants.length > 0 && (
                ...
                >
                  <div className="p-4">
                    ...
                    {getVariantAttributeGroups(item.variants).map((group) => (
                      <div key={group.key} className="mb-3">
                        <p className="mb-2 text-[10px] uppercase text-muted-foreground">{group.key}</p>
                        <div className="flex gap-2 flex-wrap">
                          {group.values.map((value) => {
                            const current = selectedVariant ? getVariantAttributes(selectedVariant.attributes) : {};
                            const candidate = item.variants.find((variant) => {
                              const attributes = getVariantAttributes(variant.attributes);
                              return attributes[group.key] === value && Object.entries(current)
                            ...
                            return (
                              <Button
                                size="icon-lg"
                                variant={candidate?._id === selectedVariant?._id ? "default" : "outline"}
                                key={`${group.key}-${value}`}
                                onClick={() => candidate && setSelectedVariants((prev) => ({ ...prev, [item.productId]: candidate }))}
                                className="text-xs tracking-wide cursor-pointer rounded-none border"
