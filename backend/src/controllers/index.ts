import * as adminController from "./admins.controller.js";
import * as categoryController from "./categorise.controller.js";
import * as couponController from "./coupon.controller.js";
import * as couponUsageController from "./couponUsage.controller.js";
import * as orderController from "./orders.controller.js";
import * as productController from "./products.controller.js";
import * as settingsController from "./settings.controller.js";
import * as subscribersController from "./subscriber.controller.js";
import * as userController from "./users.controller.js";

export {
  adminController, categoryController, couponController,
  couponUsageController, orderController, productController, settingsController,
  subscribersController, userController
};
