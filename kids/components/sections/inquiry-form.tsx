...
export default function InquiryForm({
  ...
  useEffect(() => {
    if (selectedProductId) {
      const current = getValues('interestedProductIds')
      if (!current.includes(selectedProductId)) {
        setValue('interestedProductIds', [selectedProductId])
      }
    }
  }, [selectedProductId, setValue, getValues])
  ...
  const handleProductToggle = (productId: string) => {
    ...
    if (current.includes(productId)) {
      ...
      setValue('interestedProductIds', [...current, productId])
    }
  }
  ...
  const onSubmit = async (data: InquiryFormData) => {
    ...
    try {
      const response = await axios.post('/api/kids/v1/inquiry', data)
                  ...
                  products.map((product) => (
                    <div key={product._id} className="flex items-start gap-3">
                      <Checkbox
                        ...
                        checked={getValues('interestedProductIds').includes(
                          product._id
                        )}
                        onCheckedChange={() =>
                      ...
                      >
                        ...
                        <p className="text-xs text-muted-foreground">
                          {product.fabric} • MOQ: {product.moq} pcs • ${product.priceMin}-${product.priceMax}/pc
                        </p>
                      </label>
                    </div>
                  ))
