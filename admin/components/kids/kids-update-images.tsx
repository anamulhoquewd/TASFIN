...
export default function KidsImagesForm({ product, onClose }: KidsFormProps) {
  ...
  const submit = async ({ images }: ImageValues) => {
    ...
    try {
      ...
      images.forEach((file, index) => {
        ...
        body.append(`images[${index}][alt]`, product.name);
      });
      (product.images ?? [])
      ...
      const response = await api.patch(`/kids/${product._id}/images`, body);
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(submit)} className="space-y-4">
        <Card>
          <CardHeader>
            <CardTitle>Kids product images</CardTitle>
          </CardHeader>
